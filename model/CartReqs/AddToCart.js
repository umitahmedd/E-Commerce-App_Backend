            ...
            client.query(
              ...
              (err2, result2) => {
                ...
                if (result2.rows.length > 0) {
                  ...
                  client.query(
                    "SELECT * FROM cart_product WHERE product_id = $1 AND cart_id = $2",
                    [product_id, cart_id],
                    (err3, result3) => {
                      if (err3) {
                        console.log(err3);
                        ...
                        client.query(
                          "INSERT INTO cart_product (cart_id, product_id, product_total_price, product_count) values($1, $2, $3, $4)",
                          [cart_id, product_id, product_price, 1],
                          (err4, result4) => {
                            if (err4) {
                              console.log(err4);
                        ...
                        client.query(
                          "UPDATE cart_product SET product_count = product_count + 1, product_total_price = product_total_price + $1 WHERE cart_product.cart_product_id = $2 ",
                          [product_price, cart_product_id],
                          (err5, result5) => {
                            if (err5) {
                              console.log(err5);
                            ...
                            res
                              .status(200)
                              .json({
                                message: `product with product_id: ${product_id} was added again to cart with cart_id: ${cart_id}`,
                              });
                          }
                        );
                      }
