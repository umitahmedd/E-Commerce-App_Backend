      ...
      jwt.verify(token, JWT_KEY, async (err, payload) => {
        if ((err && err.name === "TokenExpiredError") || kalansure < 900000) {
          if (!payload) {
            ...
            const userOldToken = await client.query(
            ...
            if (oldToken == token) {
              ...
              const userRefToken = await client.query(
                "SELECT user_refresh_token FROM users WHERE user_id = $1",
                [payload.user_id]
              );
              ...
              jwt.verify(
                refreshToken,
                JWT_REFRESH_KEY,
                async (refreshErr, refreshPayload) => {
                  if (refreshErr) {
                    ...
                    const newToken = jwt.sign(
                      {
                        ...
                        authorization: refreshPayload.authorization,
                      },
                      JWT_KEY,
